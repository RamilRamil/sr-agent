...
**No memory handle (strengthened 2026-07-03).** `PackContext` deliberately exposes *no* `EpisodicMemory` — a pack has no way to write memory, so it structurally cannot forge a `human_input`-tier record (FR-006 becomes a structural guarantee, not a behavioral check). The kernel owns every write and sets the source tier itself; the pack only *returns* domain artifacts (`persist_finding` → finding; `execute_confirmed` → status event) that the kernel persists. Prior findings needed by `domain_escalation` are passed to it as arguments.
