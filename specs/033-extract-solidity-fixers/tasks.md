...
- [X] T011 [US1] Create `scripts/solidity_fixers.py`; MOVE the five `_fix_*` + their private regexes +
  ...
  the pqr re-export.
- [X] T012 [US1] Verify NO import cycle (SC-009): `python -c "import scripts.solidity_fixers"` and
  `import scripts.solidity_utils` each succeed in isolation; `solidity_fixers` does NOT import
  `poc_queue_runner`.
- [X] T013 [P] [US2] Architecture test in `tests/architecture/test_fixer_sites.py` (FR-009 + SC-002):
...
- [X] T014 Run `pytest -q` — GREEN (characterization now targets `solidity_fixers`; existing `pqr._fix_*`
