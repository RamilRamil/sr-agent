...
- **FR-013**: For the "show what it's about to do" visibility required by FR-008, the system MUST display the intended consequential-but-not-irreversible action to the user and then proceed automatically without waiting for an explicit acknowledgment — this is visibility only, not a gate, and remains distinct from the hard out-of-band confirmation mandated by FR-005 for irreversible actions.
...
- **Consequential Action Notice**: The lightweight, in-conversation visibility the agent surfaces before/while taking a consequential-but-not-irreversible action, distinct from the out-of-band confirmation required for irreversible actions.
