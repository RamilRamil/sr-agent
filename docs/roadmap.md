...
The spec-022 path ran live for the first time: the Pashov strata-bb report → per-finding PoC on **Gemini** (`gemini-3.1-flash-lite`) with mainnet-**fork** verification, using the SIP2Test scaffold + h01_example few-shot + the baked `sr-agent-foundry:strata-bb` image. **3 of the 5 findings produced REAL fork-verified PoCs** (`real_pass: true` — the exploit actually triggered on a fork, not merely compiled; the `_poc_defects` gate rejected vacuous passes). Finding #1 IS the H-01 same-block-silo-padding tier-bypass we chased the entire project — the local 30B never cracked it; the harness + flash-lite got it in 3 attempts. The two failures are two DISTINCT classes (this shapes the non-compiling strategy):
...
- **Finding 2 — `Member "unstakeCooldown" not found in StrataCDO`**: the model INVENTED a contract method (gotcha #5). Not deterministically fixable (no correct name to substitute) → escalate to a stronger model (GLM 5.2) or tighter callable-api grounding.
- **Handling strategy (decided):** (1) mechanical error classes → deterministic guard; (2) invented-API/logic → escalate to a stronger model; (3) neither → quarantine to `audit/poc_failed/` + `exhausted`, never a false success. This is the "determinism beats suggestion" + two-tier (worker/smart) thesis applied to compile failures.
...
- **flash-lite over-performed**: it cracked H-01, where we assumed a frontier model was required — so GLM 5.2 (the "smart" tier) is reserved for what flash-lite can't do (e.g. finding 2's invented-API case), not used by default.
