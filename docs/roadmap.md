...
### Property/invariant-based exploit verification — **spec 035, Phases A–D LANDED (E = operator run)**
The baseline that motivated it: strata-bb 23 findings, deepseek-v3.2-exp + fork → **16 compiled_real, 4 forge-PASS, 0 passed_verified**. The wall was not only exploit-logic: `mutation_verify` needs the finding's OWN fix, and the report carries an applyable diff for just **3/23** (measured after hardening `extract_fix_for_finding` against spurious cross-finding matches, which had inflated it to 6). Worse, the sets are **disjoint** — the 3 with a fix produced no passing exploit, the 4 that passed have no fix. So `verified@23 = 0` is STRUCTURAL, and the report-fix oracle is a dead end for ~20/23.
- **The change**: a SECOND verification path whose oracle is intrinsic — the model writes an INVARIANT, a search engine looks for a violating call-sequence, and the violation IS the verification (no report fix). Trust is rebuilt to `mutation_verify`'s level by three deterministic, safe-erring gates: a COVERING honest-behaviour check (independence — the invariant must hold under legitimate use, to a coverage bar), deterministic re-confirmation, and MECHANISM ATTRIBUTION (reusing `mechanism_signal`, promoted from telemetry to a gate — without it the oracle proves a vuln EXISTS but not that it is THIS finding's, the same spurious-attribution defect the false-verified guard had just fixed for the assertion oracle). One predicate drives two actor policies (honest must hold / adversarial may break), which makes the honest check native to Foundry rather than per-test instrumentation.
- **Level-0 first, and it rewrote the spec** (spec-030 discipline). 5 hand-written invariants, 40 000-call searches: engine works on fork and found 3/5; honest-check native with coverage from forge's own Calls/Reverts table; **survival 2/5**; ~4 h for 23 → verdict **`scope-down-subset`**. Three findings that do not come from a desk: fork invariant-fuzzing is **infeasible without `targetSender()` pinning** (random per-call senders are uncacheable → provider HTTP 429 mid-run, which **presents as a hang**; 1 000 calls went 429-failure → **3.2 s**); reproduction is **bimodal** (seconds or never within 40 000 calls), so budgets are sized by the "found fast" fraction, not a mean; and for the rounding/accounting class the honest-check event and the bug event are the SAME event.
- **That last one produced the sharpest requirement.** Recovering the honest-manifesting class naively (promote a reproduced + attributed honest violation) re-opens the unsafe mode: rounding is DETERMINISTIC so it always reproduces, and the honest path calls exactly the functions the finding names so attribution always matches — a merely over-strict invariant would earn a trustworthy verdict. The discriminator therefore had to be one the model does NOT author: **does the discrepancy ACCUMULATE** across repeated honest operations (a rounding artifact stays bounded; a leak grows), with any external threshold sourced from the TARGET's own artifacts, never the model. Absent measurement → not material (never promote on ignorance). A known residual gap is stated rather than hidden: constant-magnitude honest-manifesting bugs are not recovered, and are reported "out of subset", never "not confirmed".
- **Eval comparability held on both axes**: the headline `verified` number stays assertion-only (comparable to every prior run) with the invariant oracle on a SEPARATE labelled axis, and — because the subset is curated by predicted survivability — every published interval must carry its subset size, selection rule, and the full-set consequence with excluded case-runs counted as unverified. A bare subset share cannot be published. The calibration threshold that declares the new oracle UNCALIBRATED is fixed in advance, not fitted afterwards.
- Landed: Phase B (engine-agnostic classifier), C (loop wiring, opt-in behind `--invariants` so the assertion path is byte-unchanged), D (eval axis + calibration). **6 review rounds, 13 risks raised, 13 closed** — the last three requirements exist because Level-0 measured them, not because they were reasoned.

### Extract the deterministic compile-fixer layer — **LANDED as spec 033**
A behavior-preserving refactor, no functional change. The import-path bug class had recurred 3× in one day (029 smoke-import, 031 synth-base `../` depth, 032 wiring) because the deterministic fixers were scattered through the ~3060-line `poc_queue_runner.py` and two near-duplicate repair loops (031 synth-synthesis, 032 drafting in-place) hand-inlined their transform sequences — and that duplication made a 032 mutation-test accidentally target the wrong loop. The fix gives each fixer ONE home so a logic fix applies everywhere.
- **The change**: the six deterministic fixers (`_fix_import_paths`/`_nested_type_imports`/`_address_interface`/`_undeclared_import`/`_setup_override`/`_scaffold_base`) + their private regexes + four named sequence-functions (one per site's sequence; `_seq_postmodel` serves both post-model sites, byte-identical) move to `scripts/solidity_fixers.py`; the shared low-level helpers they pull in that grounding/scaffold also use (`_tracked_sol`/`_SKIP_DIRS`/`_path_for`/`_strip_comments`/`_scaffold_base_name`/`POC_SUBDIR`) move to `scripts/solidity_utils.py` to break the would-be import cycle. `poc_queue_runner.py` re-exports transitionally; the two loops now call the named sequence-functions.
- **T001 correction (during implement)**: the 5-round spec review still undercounted — `_fix_scaffold_base` is a SIXTH fixer (part of both post-model sequences), and the post-model sites apply FOUR fixers each with its own event, not the spec table's three. A byte-identical move must reproduce the real code, so the artifacts were corrected before extracting.
- **Guarded in four reviewable-no-op commits** (extract → swap+drop-gate → move-utils → move-fixers): characterization tests pin each sequence's output (incl. the drafting in-place site's intentional ABSENCE of `import_paths`, and the synth `base_dir=synth_dir` depth); a TEMPORARY differential gate (FR-014) proved each extracted function byte-identical to the real loop's inline output by CAPTURING the artifact the loop writes (never transcribing — a transcription would agree on a mis-copied `base_dir`); a name-keyed architecture test (`tests/architecture/test_fixer_sites.py`) enforces the site inventory (a sixth site fails it until the set is consciously updated) + SC-002 (no `def _fix_*` left in pqr). Both guards verified mutation-fit. Full suite green at every commit (712 passed). The SEQUENCE UNIFICATION (does giving the drafting in-place step `import_paths` improve compile-convergence?) is deferred to the **034** stub — it CHANGES behavior and needs its own evidence.

### Deterministic compile-fixers in the drafting loop — **LANDED as spec 032**
Two capable models (GLM-5.2, deepseek-v3.2), up to 8 attempts, both cleared the COMPILE wall only intermittently and NEVER landed the exploit — the cheap capability levers are EXHAUSTED and NEGATIVE (more attempts, a different frontier model, and even a SUFFICIENT scaffold — finding-2 compiled 2/8, each a DIFFERENT compiler error). Root cause of the compile non-convergence: the loop relies on the MODEL to apply mechanical fixes it is told about via `_targeted_hints`, and the model keeps making new mechanical mistakes. Measured compile-error frequency (eval-first, from the run logs): undeclared-identifier (7576/7920) ×8 (dominant), address→interface (9553) ×3 — both MECHANICAL; wrong-arg-count ×3, invalid-token ×2, cannot-instantiate-interface ×1 — SEMANTIC, not fixable.
- **The change**: the harness fixes the two mechanical classes ITSELF. New `_fix_undeclared_import` auto-imports an undeclared name ONLY when `_path_for(file_map, X)` resolves it to a real top-level symbol (anti-invention: a typo/invented name is left for the model — the name is read from under the error's `^^^` caret, since 7576/7920 omit it from the message). The existing 9553 `_fix_address_interface` (spec 031, synth-only) is now ALSO applied in the drafting loop. Both run as a BOUNDED IN-PLACE deterministic-repair sub-step (`DET_REPAIR_ROUNDS`=2) on the compile-FALSE branch, keyed on the FAILING compile's own output (line numbers valid — running after the model rewrites the code would drift `_fix_address_interface`'s line anchor), recompiling in-place and accepting the moment it compiles.
- **A1 remediation (from /speckit-analyze)**: the sub-step does NOT advance the `for attempt` counter, so a mechanical fix does NOT consume the `--attempts` budget — it must not starve the model's exploit-logic attempts. Idempotency + the round cap terminate it. A `deterministic_fix` event records what the harness repaired. NO model call added → **Constitution V strengthened** (fewer model round-trips; the harness converges the compile deterministically). Semantic classes stay with the model + `_targeted_hints`. The exploit-LOGIC wall (the `assertion failed` / access-control class) is the remaining direction — item "b", the fuzzing/symbolic hybrid (model writes an invariant-harness, an engine searches for the triggering input), a separate future spec.

### Harden scaffold synthesis (deterministic repair pass) — **LANDED as spec 031**
With a capable model past the compile wall (029), the exploit-logic run repeatedly stalled on the DEPLOY scaffold, not the exploit. A deterministic Level-0 opportunity check (no model) over the 5 curated findings found **0/5** opportunity for finding-aware base RANKING (spec 030): one shipped deploy base is BOTH most-inherited AND best-fitting for every finding, and the genuine gap (a cooldown contract absent from EVERY base) needs SYNTHESIS, which ranking cannot replace — so **030 was deferred** (sound, but unmeasurable on the only eval target; revisit on a multi-base target). The eval-first check (its own principle) caught that before implementation. The measurable lever here is instead **making synthesis reliable**: it was ONE-SHOT (`synthesize_scaffold` compiled the smoke test once, discarded the base on any `no_build`), while the drafting PoC gets N deterministic repair rounds — so a synth base died on a single mechanical error (live: solc 9553 `Invalid implicit conversion from address to contract IFoo`, the model passing `setter(address(x))` for an interface param).
- **The change**: a BOUNDED (`SYNTH_REPAIR_ROUNDS`=3) DETERMINISTIC repair loop around the smoke build — on `no_build`, apply the harness's deterministic transforms (`_fix_import_paths` + `_fix_nested_type_imports` + a new error-driven `_fix_address_interface` for 9553), rewrite the base, re-compile; accept the moment it compiles, early-stop when a round changes nothing, honest give-up after the bound. Plus a shared `_targeted_hints` 9553 rule (helps the drafting PoC too). Key design point: `_targeted_hints` returns HINTS (for the model) but a no-model synth repair needs TRANSFORMS — so 9553 got both a code transform (synth) and a hint (PoC). NO model calls in the repair → **Constitution V strengthened** (more reliable, no new model dependency); acceptance bar unchanged (trust only a compiling base — feature 011 FR-004).
- **Eval designed up front** (the 030 lesson): deterministic unit tier (repair accept/early-stop/bound + the 9553 transform/hint, forge/model stubbed) + a live synth-compile corroboration tier. Fixtures from a real forge 9553 shape renamed to invented placeholders (no target material). Guards mutation-proved (on committed code — reverse Edit, never `git checkout` uncommitted work, a lesson from 029). Invented-API (a base calling a non-existent function) stays OUT of scope (not deterministically fixable).

### Trace-grounded exploit-logic feedback — **LANDED as spec 029**
The compile wall fell the moment a capable model ran. First proof-eval run on a paid model (GLM-5.2 via OpenRouter): flash-preview/north-mini-code compiled 0/5 (hallucinated the API despite AST-grounded `callable_api` + exact struct/enum defs — a MODEL ceiling, not a grounding gap); GLM-5.2 compiled a real PoC that deployed the whole protocol on a fork on attempt 1. The NEW bottleneck is exploit LOGIC: the PoC compiles and runs but the exploit assertion fails (the attack is correctly BLOCKED), and the harness fed only the `[FAIL: …]` line + finding text — so the model re-guessed the trigger blind. This matches the literature (PoCo arXiv:2511.02780, A1 arXiv:2507.05558): **execution-trace feedback in the refinement loop**, not more pre-digested grounding, is the lever (PoCo zero-shot 0/23 → agentic loop 16/23; ~60–70% of successes need multiple cycles).
- **The change**: the drafting-loop forge run opts into `-vvv` (traces for FAILING tests only — passing tests get none, so no noise to trim), a budget-bounded `_trace_excerpt` (anchored on `[FAIL…]`, keeps the `Traces:`/`Backtrace:` region, revert-side tail preferred under pressure), folded into `revert_hints` AFTER the authoritative setup-revert fix and BEFORE the generic guidance. Empty trace → byte-identical to the prior output (graceful degradation). Captured in the SAME run via verbosity → no extra fork execution; verdict unchanged (`-vvv` doesn't touch the `[PASS]`/`[FAIL]`/`Compiler run failed` markers). Confined to the loop via an additive default-off `run_tests(trace=…)` param — `mutation_verify` and the scaffold smoke untouched (FR-010).
- **Constitution-safe**: model-agnostic — a better observation signal helps ANY client (local/free/paid), so no paid-API dependency (Principle V). Fixtures built from a REAL captured `forge -vvv` snippet, renamed to invented placeholders (no target material; `test_no_target_material` green). Two harness bugs found + fixed in passing along the way: the mutation-verify `node_modules` read-only mount (a `run_tests` `extra_mounts` param — a host-path symlink dangles inside the container), and the scaffold-synthesis import DEPTH (`_fix_import_paths` gained a `base_dir` — the synth base sits a level deeper than `audit/poc/`, so the model's `../` count was off by one; computing a path is mechanical, not the model's job).
