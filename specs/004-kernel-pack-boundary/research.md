...
## R13 — Post-hoc validation against the PoC-writing workability test (2026-07-02)

**Decision**: No change to the kernel/pack placement design (R1–R12) — a live end-to-end test (local model reads a real audit report, drafts Foundry PoCs, runs them in `DockerSandbox` against real contracts) confirmed every placement call held: the bugs hit were either genuinely kernel-mechanism-level (sandbox invocation safety — see the `PackContext.sandbox` note in data-model.md) or genuinely Foundry/audit-specific (offline `solc` caching, `--offline`/`--use` flag interaction, per-project baked sandbox images, PoC-file quarantine against Foundry's whole-directory compile semantics) — all squarely inside `write_execute.py`'s move-to-pack scope. `LocalClient`'s real-world hardening (switching `generate()` to streaming to survive a proxy/tunnel's idle-connection timeout) landed exactly where R12-adjacent placement already put it (kernel).

**Rationale**: This is the closest thing to an empirical check the design gets before implementation. The one genuine gap found — `DockerSandbox.run()`'s shell-form-entrypoint pitfall was undocumented at the kernel level, living only in the soon-to-be-relocated `write_execute.py` — is now captured in `data-model.md`/`contracts/pack-interface.md` so a second pack (or a second audit-pack tool) doesn't rediscover it. PoC-drafting *quality* (few-shot grounding from existing repo tests, transitive-import context, verbose revert traces) is confirmed to be pack **content** (`packs/audit/reasoning.py`'s future `reasoning_prompt`), tunable independently of this structural feature — not a Spec 004 concern.

**Alternatives considered**: N/A — this is a validation entry, not a new design fork. Full detail (11 reproduced infra gotchas) lives in `docs/roadmap.md`'s knowledge-base section, recorded there per Constitution IV (human-gated) rather than duplicated here.
