...
def ingest_response(
    ...
    try:
        raw_findings = extract_findings(response_text)
    ...
    for idx, raw in enumerate(raw_findings):
        ...
        # status_change is structurally not a Finding field — silently dropped
        # here, so a relayed response can never carry a privileged status.
